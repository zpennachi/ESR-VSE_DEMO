...
const CAMERA_STATE_LEFT = -2;
const CAMERA_STATE_RIGHT = 2;
...
document.getElementById('leftArrow').addEventListener('click', () => {
    if (isBehindBasket) {
        if (currentCameraState < CAMERA_STATE_RIGHT) { // If not already rightmost
            currentCameraState++;
            updateCameraTargetSmoothly(currentCameraState * 3, 1000); // Move right
        }
    } else {
        if (currentCameraState > CAMERA_STATE_LEFT) { // If not already leftmost
            currentCameraState--;
            updateCameraTargetSmoothly(currentCameraState * 3, 1000); // Move left
        }
    }
});
...
function setCameraPosition(position) {
    ...
    switch (position) {
        case 1:
            ...
            break;
        case 2:
            targetPosition = new THREE.Vector3(-6, -2, 9);
            isBehindBasket = false;
            controls.target.set(0, 1, 10); // Adjust the values according to your scene
            currentCameraState = CAMERA_STATE_CENTER;
            break;
        case 3:
